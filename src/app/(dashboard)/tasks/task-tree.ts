...
export function duplicateEntry(entries: SubtaskEntry[], id: string): SubtaskEntry[] {
    return entries.flatMap((entry) => {
        if (entry.id === id) {
            return [entry, { ...cloneEntry(entry), entries: entry.entries.map(cloneEntry) }];
        }
        if (entry.entries.length > 0) {
            return [{ ...entry, entries: duplicateEntry(entry.entries, id) }];
        }
        return [entry];
    });
}

function cloneEntry(entry: SubtaskEntry): SubtaskEntry {
    return {
        ...entry,
        id: `se-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`,
        entries: entry.entries.map(cloneEntry),
    };
}
